...
public interface DeveloperTrainingModuleRepository extends AbstractRepository {
	...
	@Query("SELECT ts FROM TrainingSession ts WHERE ts.periodStart = (SELECT MIN(ts2.periodStart) FROM TrainingSession ts2 WHERE ts2.trainingModule.id = :id)")
	Collection<TrainingSession> findTrainingSessionsWithEarliestDateByTrainingModuleId(int id);
