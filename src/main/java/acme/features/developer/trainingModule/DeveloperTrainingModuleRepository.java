...
public interface DeveloperTrainingModuleRepository extends AbstractRepository {
	...
	@Query("SELECT ts FROM TrainingSession ts WHERE ts.trainingModule.id = :id AND ts.periodStart < (SELECT tm.creationMoment FROM TrainingModule tm WHERE tm.id = :id)")
	Collection<TrainingSession> findTrainingSessionsWithPeriodStartBeforeTrainingModuleCreationMoment(int id);
