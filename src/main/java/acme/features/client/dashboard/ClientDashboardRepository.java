...
public interface ClientDashboardRepository extends AbstractRepository {
	...
	@Query("select count(p) from ProgressLog p where  p.completeness < :higher and p.completeness >= :lower and p.client.id = :id")
	int logsBetweenCompletenessValuesForClient(int id, float lower, float higher);

	@Query("select count(p) from ProgressLog p where  p.completeness < :higher and p.client.id =:id")
	int logsBelowCompletenessValue(int id, float higher);

	@Query("select count(p) from ProgressLog p where  p.completeness >= :lower and p.client.id = :id")
