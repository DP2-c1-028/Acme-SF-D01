...
public interface ClientProgressLogRepository extends AbstractRepository {
	...
	@Query("select p2 from ProgressLog p2 where p2.draftMode = false and p2.contract.id = :id and p2.completeness = (select max(p.completeness) from ProgressLog p where p.contract.id = :id and p.draftMode = false)")
	ProgressLog findContractProgressLogWithMaxCompleteness(int id);
